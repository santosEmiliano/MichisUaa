...
const readSightings = async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ mensaje: "Error al obtener los avistamientos" });
    }
}
...
const readSightingsById = async (req, res) => {
  try {
    ...
    if (!sighting) {
      return res.status(404).json({ mensaje: "Avistamiento no encontrado, verifique la id" });
    }
    
    return res.status(200).json(sighting);
  } catch (error) {
    console.error("Error al obtener avistamiento:", error);
    return res.status(500).json({ mensaje: "Error al obtener el avistamiento" });
  }
};
...
const registerSighting = async (req, res) => {
  try {
    ...
    return res.status(500).json({ mensaje: "Error al registrar el nuevo avistamiento" });
  }
};
...
const modifySighting = async (req, res) => {
  try {
    ...
    if (!oldSighting) {
      return res.status(404).json({ mensaje: "Avistamiento no registrado para actualizar" });
    }
    ...
    const updatedSighting = await sightingFunctions.modifySighting(id, req.body);
    ...
    return res.status(500).json({ mensaje: "Error al actualizar el avistamiento" });
  }
};
...
const deleteSighting = async (req, res) => {
  try {
    ...
    if (!existSighting) {
      return res.status(404).json({ mensaje: "Avistamiento no encontrado para eliminar, verificar el id" });
    }
    ...
    return res.status(200).json({ mensaje: "Avistamiento eliminado correctamente" });
  } catch (error) {
